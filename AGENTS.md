...
- **阶段**: RTX PRO 6000 复测 MTP baseline（2.06× L40S）；DFlash 新卡复测仍不适用；DSpark 经 AEON 同卡实测确认单卡仍输 MTP（+11% over DFlash → ~0.84× MTP）→ 转向：先攻 MTP k≥4 崩溃，再评估自训 drafter
...
- **MTP k≥4 验证（2026-07-09 + 0.25.1/RTX PRO 6000 复测仍崩→转「主动解决」）**: k=4/k=5 默认在并发 ≥5 必触发 `cudaErrorIllegalAddress`（FlashInfer + spec-decode 强制 PIECEWISE cudagraph，k≥4 越界，**非显存**）；0.25.1 新卡复测仅 5 请求烟测过，完整 benchmark 100% 失败。已试无效：`--enforce-eager`（慢 25-46%）、max-num-seqs=5、PR #46324 补丁。**待验证假设**：根因在 FlashInfer 强制 PIECEWISE，切 `--attention-backend FLASH_ATTN`（支持 FULL cudagraph，AEON 同款 RTX PRO 6000 用此跑通）或可绕过。详见 `docs/performance.md`「MTP k≥4 验证」章节
...
- **下一步（2026-07-23 修订，按性价比排序）**:
  1. **解决 MTP k≥4 崩溃**（首选；零新增 drafter 开销的纯收益）: 待验证假设——根因在 FlashInfer 强制 PIECEWISE cudagraph，尝试切 `--attention-backend FLASH_ATTN`（支持 FULL cudagraph，AEON 同款 RTX PRO 6000 用此跑通）绕过 k≥4 越界 bug；若成功则可比 k=3 提升 accepted length。`enforce-eager` / PR #46324 已证无效。
  2. **自训 DSpark drafter**（次选；针对 stock Qwen3.6-27B-FP8 / EN+ZH）: 用 AEON 开源 recipe（z-lab DFlash 基础 + rank-256 Markov 头，on-policy 蒸馏，单卡可训）。但同卡实测 DSpark≈0.84× MTP，预期仍输 MTP；自训主价值是拿本项目口径的实测数，而非翻转结论。
  3. **翻转 DSpark 结论的真正前提**（非近期）: 多卡 / disaggregation 给 drafter overlap 空间，或 acceptance 量级跃迁——而非更强的 drafter 算法或更高 k。
