...
- **当前配置**: MTP k=3 + baseline 参数 + 容器 host 网络/ ipc host 优化（Stage 0 验证最优，容器优化验证等价裸机）
...
- **下一步**: DSpark 路线在单 L40S + 27B 配置下已穷尽（裸机部署方向已排除），待硬件升级（多卡 ≥2×46GB 或 80GB）+ qwen3_5 draft model 实现后重启
