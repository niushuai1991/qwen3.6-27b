...
- **阶段**: Baseline（MTP k=3，默认参数）
- **GPU 显存**: ~39.5/46 GB
- **基线性能**: output_tps = 36.3(c=1) / 170.0(c=5) / 309.8(c=10)，0 失败
- **已完成**: Task 1（安装依赖）、Baseline benchmark
- **进行中**: Task 2 — Stage 0 参数优化
  - `max-num-partial-prefills` / `max-long-partial-prefills`: vLLM 0.24 不支持
  - `enable-flashinfer-autotune` / `max-num-batched-tokens>8192` / `gpu-memory-utilization=0.92`: 任一变更导致 benchmark 全部请求失败（单次 curl 却成功）
  - 详细记录见 `docs/performance.md#stage-0-尝试记录`
- **建议下一步**: 逐个隔离测试参数变更；或跳过 Stage 0 直接到 Stage A (DFlash)
